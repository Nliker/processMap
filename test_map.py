...
if __name__ == "__main__":
    ...
    for file_name in [file for file in image_list if file.endswith('.png')]:
        ...
        binary_map=np.loadtxt(f"{binary_dir}/{binary_name}", delimiter=' ',dtype = 'int')
        walkable_spot_tuple = np.where(binary_map==1)
        walkable_spot_list= [(walkable_spot_tuple[0][i],walkable_spot_tuple[1][i]) for i in range(len(walkable_spot_tuple[0]))]
        start=walkable_spot_list[0]
        end=walkable_spot_list[100]
