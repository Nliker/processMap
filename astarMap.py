...
def get_shortest_path(map:list[list[int]],start:tuple,end:tuple)->tuple[list, float]:
    h=len(map)
    w=len(map[0])
    ...
    heap = []
    total_cost = 0
    heapq.heappush(heap, (heuristic_cost[start[0]][start[1]] + total_cost, start[0], start[1]))
    cnt=1
    while heap and (heap[0][1:])!=end:
        total_cost, x, y = heapq.heappop(heap)
        # Total Cost 에서 휴리스틱 코스트를 빼면 시작 지점에서 현재 지점까지의 실제 거리를 구할 수 있음
        if total_cost_list[x][y] < total_cost:  # 기존에 있는 거리보다 길다면, 볼 필요도 없음
            continue  
